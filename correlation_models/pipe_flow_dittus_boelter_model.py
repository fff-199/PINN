...
def calc_zone_properties(temp_c, nu_n2):
    ...
    # ============ 气相扩散系数 ============
    d_ab = 0.7e-5 * ((temp_k / 298) ** 1.75)

    # ============ 无量纲数计算 ============
    re = V_air_effective * Dh / nu_n2
    sc = nu_n2 / d_ab
...
# =========================================================================
print('\n' + '=' * 70)
print('蒸发干燥模拟结果 (双面蒸发模式 - 上下两面同时干燥)')
print('=' * 70)
print(f"\n【模型配置】")
print(f"  蒸发面数: {N_surfaces} (上下两面)")
print(f"  单面涂布面积: {Area_drying_total:.4f} m²")
print(f"  有效蒸发面积(双面): {Area_drying_effective:.4f} m²")
print(f"  面积放大倍数: {N_surfaces}×")
print(f"\n【初始条件】")
...
# =========================================================================
# Plotting - Styled similar to "Turbulent Flat Plate Correlation.py"
# =========================================================================
...
# 设置标题和坐标轴
ax1.set_title(f'Drying Kinetics (Double Surface) - 20 m/min\n'
...
ax.set_title('Mass Transfer Coefficient', fontsize=12, fontweight='bold')
ax.grid(axis='y', alpha=0.3)
...
ax.set_ylabel('Reynolds Number (x1e5)', fontsize=11, fontweight='bold', color='#7E57C2')
ax2_sub.set_ylabel('Sherwood Number', fontsize=11, fontweight='bold', color='#26A69A')
ax.set_title('Dimensionless Numbers', fontsize=12, fontweight='bold')
...
fig3.suptitle('Evaporation Parameters Summary', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
