...
![PINN综合可视化](./pinn_visualization_comprehensive.png)

上图包含 6 个面板：
*   **(a) 浓度场云图**: 边界层内的浓度分布，白色虚线标记边界层厚度 δ(x)。
*   **(b) 训练损失曲线**: Loss 从 1 下降到 <10⁻⁴，证明模型收敛。
*   **(c) 浓度剖面**: x=1m, 4m, 7m 处的垂直剖面，展示梯度随位置变缓（传质阻力增加）。
*   **(d) 表面蒸发通量**: J(x) 沿程衰减，直观体现“传质阻力”效应。
*   **(e) 边界层厚度增长**: δ(x) 从入口约 50mm 增长到出口约 250mm。
*   **(f) PINN 架构图**: 神经网络结构和损失函数公式。
